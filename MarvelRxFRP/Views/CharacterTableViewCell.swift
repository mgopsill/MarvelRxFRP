...
class CharacterTableViewCell: UITableViewCell {
    ...
    func bind(using character: MarvelCharacter) {
        let (name, image) = characterCellViewModel(character: character)
        name.drive(title.rx.text).disposed(by: disposeBag)
        image.drive(characterImageView.rx.image).disposed(by: disposeBag)
    }
...
func characterCellViewModel(
    ...
    image: Driver<UIImage?>
        ...
        return (
            name: Driver.of(character.name),
            image: Current.images.image(character.imageURL).asDriver(onErrorJustReturn: nil)
        )
}
